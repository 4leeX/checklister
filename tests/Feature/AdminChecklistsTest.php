...
class AdminChecklistsTest extends TestCase
{
    ...
    {
        $checklists_group = ChecklistGroup::factory()->create();

        $checklists_url = 'admin/checklist_groups/' . $checklists_group->id . '/checklists';
        ...
        $response = $this->post($checklists_url, [
            'name' => 'First checklisit'
        ]);
        $response->assertRedirect('welcome');

        $checklist = Checklist::where('name', 'First checklisit')->first();
